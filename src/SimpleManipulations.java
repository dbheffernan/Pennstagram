...
public class SimpleManipulations {
    ...
    public static PixelPicture rotateCCW(PixelPicture pic) {
        ...
        for (int x = 0; x < h; x++) {
            for (int y = 0; y < w; y++) {
                tgt[x][w - y - 1] = src[y][x]; // swap coordinates
            }
        }
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                    tgt[x][y] = new Pixel((255 - (src[x][y].getRed())),(255 - (src[x][y].getGreen())),(255 - (src[x][y].getBlue()))); 
            }
        }
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                    int grey = (int)Math.round ((src[x][y].getRed() + src[x][y].getGreen() + src[x][y].getBlue()) / 3.0);
                    tgt[x][y] = new Pixel(grey, grey, grey); 
            }
        }
    ...
    public static PixelPicture scaleColors(
            ...
            for (int x = 0; x < w; x++) {
                for (int y = 0; y < h; y++) {
                        tgt[x][y] = new Pixel((int)Math.round (rfactor * (src[x][y].getRed())),(int)Math.round 
                                                            (gfactor * (src[x][y].getGreen())),(int)Math.round 
                                                            (bfactor *  (src[x][y].getBlue()))); 
                }
            }
    ...
    public static PixelPicture alphaBlend(
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                    tgt[x][y] = new Pixel(weightedAverage(alpha,src[x][y].getRed(), src2[x][y].getRed()),
                                          weightedAverage(alpha,src[x][y].getGreen(), src2[x][y].getGreen()),
                                          weightedAverage(alpha,src[x][y].getBlue(), src2[x][y].getBlue())); 
            }
        }
