public class AdvancedManipulations {
     ...
     */
    public static int contrastHelper(int val, double multiplier, int avg){
        int contrast = (int) Math.round(((val - avg) * multiplier) + avg);
        return contrast;
    }
    public static PixelPicture adjustContrast(
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                    totalIntense += (src[x][y].getRed() + src[x][y].getGreen() + src[x][y].getBlue()); 
                    totalPix++;
            }
        }
        int avgIntense = (int) Math.round(totalIntense / (totalPix*3.0));
        
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                    tgt[x][y] = new Pixel((contrastHelper(src[x][y].getRed(), multiplier, avgIntense)),
                            (contrastHelper(src[x][y].getGreen(), multiplier, avgIntense)),
                            (contrastHelper(src[x][y].getBlue(), multiplier, avgIntense))); 
            }
        }
    ...
    public static PixelPicture reducePalette(PixelPicture pic, int numColors) {
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                if(colorMap.contains(src[x][y]) == false)
                    colorMap.put (src[x][y],1);
                else
                    colorMap.put(src[x][y], (colorMap.getValue(src[x][y])+1));
            }
        }
        Pixel[] sorted = colorMap.getSortedPixels();
 for(int i =0; i<3; i++){
     sorted[i].toString();}
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                ...
                for (int z = 0; z < numColors; z++){
                    if(src[x][y].distance(sorted[z]) < closest[0]){
                        closest[0] = src[x][y].distance(sorted[z]);
                        closest[1] = z;}
                }
                tgt[x][y] = sorted[closest[1]]; 
            }
        }
        return new PixelPicture(tgt);
    }
    ...
    public static PixelPicture blur(PixelPicture pic, int radius) {
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                src[x][y].toString();
                tgt[x][y] = average(radius, x, y, h, w, src);
            }
            }
        return new PixelPicture(tgt);
    }

    public static Pixel average(int radius, int x, int y, int h, int w, Pixel[][]src){
        int cells=0;
        int xStart = (x-radius);
        int yStart = (y-radius);
        ...
        int[] pixelTotal = {0,0,0};
            ...
            for(int xCount = clipper(xStart, w); xCount < (clipper(xFinish, w-1)); xCount++)
                for(int yCount = clipper(yStart, h); yCount < (clipper(yFinish, h-1)); yCount++){
                    pixelTotal[0]+= src[xCount][yCount].getRed();
                    pixelTotal[1]+= src[xCount][yCount].getGreen();
                    pixelTotal[2]+= src[xCount][yCount].getBlue();
                    cells++;
                }
            
            Pixel tgt = new Pixel ((int)Math.round(pixelTotal[0]/cells),(int)Math.round(pixelTotal[1]/cells),(int)Math.round(pixelTotal[2]/cells));
        return tgt;
    }
    
    public static int clipper (int num, int max){
        if (num < 0)
            num =0;
        else if (num > max)
            num = max;
